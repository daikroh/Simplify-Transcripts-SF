...
class AgendaItemDetail(APIView):
    def get(self, request):
        ...
        if not agenda_id:
            return Response(
                {"error": "Missing agenda_id in query parameters"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not queryset.exists():
            return Response(
                {"error": "Agenda item not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class Search(APIView):
    def get(self, request):
        ...
        if not query:
            return Response(
                {"error": "Missing q in query parameters"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class RecordListOfAgenda(APIView):
    def get(self, request):
        ...
        if not record_id:
            return Response(
                {"error": "Missing record_id query parameter"},
                status=status.HTTP_400_BAD_REQUEST,
            )
